...
- **Status:** Active.
- **Amendment (2026-04-23, commit TBD): scope extended to cover the post-T-009 struct shape.** T-009 added two `u64` fields to `QemuVirtCpu` — `frequency_hz` and `resolution_ns` — populated once in `new()` from `MRS CNTFRQ_EL0` and the derived round-to-nearest resolution. The original body's *Invariants relied on* line "`QemuVirtCpu` is a zero-size type with no fields" no longer holds; this Amendment records the new invariants explicitly without rewriting the original entry (per unsafe-policy §3 append-only).
