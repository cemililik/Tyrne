...
7. **[T-014 — Idle dispatch via separate fallback slot](../../analysis/tasks/phase-b/T-014-idle-dispatch-fallback.md)** *(Draft 2026-05-06)*. Will refactor `Scheduler<C>` to add `idle: Option<TaskHandle>` field + `register_idle` raw-pointer free fn; update dispatch sites (`start_prelude`, `yield_now`, `ipc_recv_and_yield`) to consult `idle` only when the ready queue is empty; update `bsp-qemu-virt::kernel_entry` to call `register_idle`. Estimated diff: ~30-50 LOC kernel + 1 BSP `kernel_entry` line + 2-3 new host tests + smoke pass + UNSAFE-2026-0014 Amendment + UNSAFE-2026-0019/0020 final Amendments. Promoted `Draft → In Progress` once picked up; `In Progress → In Review` on smoke pass; maintainer flips to `Done` after independent verification.
