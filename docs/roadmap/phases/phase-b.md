...
**Status: B1 implementation complete 2026-04-28** with PR #10's merge to `main`. T-013 (EL drop) Done 2026-04-27 via PR #9; T-012 (exception infrastructure + IRQ delivery + idle WFI) Done 2026-04-28 via PR #10. **B1 closure review trio is pending** — a consolidated business + security + performance-baseline pass per the trigger in [`docs/roadmap/current.md`](../current.md). Maintainer-side QEMU smoke + Miri pass also remain pre-closure work-items.
