...
**QEMU smoke trace** — literal serial output at the post-T-014 HEAD; `-d int,unimp,guest_errors` window empty for the ~6.3 ms boot-to-end run; no exception, no PSCI call, no panic:
...
**Audit-log Amendments appended.** UNSAFE-2026-0019 / 0020 each gained a 2026-05-06 *post-T-014 smoke* Amendment confirming setup sites under sustained execution; `Pending QEMU smoke verification` status persists for both because the IRQ-take + dispatch path itself is not exercised by the v1 demo (no `arm_deadline` caller). UNSAFE-2026-0021's status persists unchanged — the demo continues to provide no deadline arm. UNSAFE-2026-0014 also gained a 2026-05-06 Amendment naming `register_idle` as the new sanctioned site for the same momentary-`&mut` pattern (consistent with the prior `start_prelude` Amendment from 2026-04-27 and the `irq_entry` Amendment from 2026-04-28).
