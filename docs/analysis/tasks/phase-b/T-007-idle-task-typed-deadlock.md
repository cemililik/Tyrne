...
| 2026-04-22 | @cemililik (+ Claude Opus 4.7 agent) | Post-In-Review second-read pass — applied the T-006-mini-retro DoD gate. Two material gaps surfaced and were routed (not closed in T-007 itself; both became §"Deferred follow-ups" entries on this task file): (1) `ipc_send_and_yield` has no symmetric state-restore test analogous to the new `ipc_recv_and_yield_returns_deadlock_when_ready_queue_empty` — the send-side bridge has the same "Err leaves state unchanged" implicit contract but no direct assertion. Routed to **T-011** (missing-tests bundle, B0 Draft). (2) `idle_entry`'s `.expect("idle: yield_now failed")` is correct today (v1 cooperative — `NoCurrentTask` is impossible once the scheduler started) but its failure modes widen under preemption. Routed to the **preemption design ADR** whenever it is written. No code changes from this pass — the second-read found that the implementation is materially complete and the residual items are forward-deferred work items, not gaps in T-007's own deliverable. |
| 2026-04-27 | independent approval-review agent | Close-out review pass for `In Review → Done`. Re-ran every gate from a clean checkout: 130 host tests + miri 130/130 + fmt/clippy/build/QEMU smoke all green. Verified idle is registered third in `kernel_entry` (B → A → idle FIFO order); ADR-0022 Accepted; `SchedError::Deadlock` and `IpcError::PendingAfterResume` variants present and tested. Audit-log compliance clean (no new audit entries from T-007 — idle reuses existing `TaskStack` pattern). Cross-references in current.md / phase-b.md / task index all consistent at `In Review`. Verdict: APPROVE. Promoting to `Done` in this commit. |
