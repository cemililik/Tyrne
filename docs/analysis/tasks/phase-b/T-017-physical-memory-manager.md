...
1. **`kernel/src/mm/pmm.rs` — `Pmm` struct + bitmap arithmetic + `Pmm::new` constructor.** Reservation marking; counter init; hint-pointer init. Pure safe Rust; bitmap operations are `[u8]` integer arithmetic. Host tests added: `new_marks_reserved_ranges_and_initialises_counters`. No `unsafe` in this commit.
2. **`kernel/src/mm/pmm.rs` — `alloc_frame` / `free_frame` / `stats`.** Adds the frame-zeroing `unsafe` block (covered by UNSAFE-2026-0001 Amendment). Host tests added: alloc/free round-trip + `OutOfFrames` + `DoubleFree` rejection + recovery-after-free + `stats` parity. The Amendment lands in `docs/audits/unsafe-log.md` in this commit.
