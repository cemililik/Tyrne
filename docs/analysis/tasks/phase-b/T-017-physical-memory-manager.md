...
[ADR-0035](../../../decisions/0035-physical-memory-manager.md) settles the PMM design: bitmap allocator with hint pointer, one bit per frame, reservation tracking via `Pmm::new(extent, &reserved)`, no `unsafe` in the PMM body except a single frame-zeroing site that lands under [UNSAFE-2026-0026](../../../audits/unsafe-log.md) — a **new** audit-log entry per ADR-0035 §Dependency chain step 5's adjudication-deferred verdict (PL011 MMIO base blessing in UNSAFE-2026-0001 and PMM RAM zero-fill share surface shape but differ enough on what they touch / what proves ownership / what the failure mode is that the umbrella stretch would violate unsafe-policy.md §3's "same safety argument" requirement). The four constraints driving the choice — bounded RAM (32 K frames) makes O(N) scan acceptable; lowest metadata footprint (4 KiB) of any option; forward-portable to high-half kernel; trivial reservation tracking — are recorded in the ADR's §Decision drivers + §Decision outcome.

T-017 is the implementation of those decisions per the ADR's §Dependency chain: a single bundled task covering the new `kernel/src/mm/pmm.rs` module + BSP wiring + host tests + new audit-log entry (UNSAFE-2026-0026, per the §Dependency-chain-step-5 adjudication-deferred verdict) + companion architecture-doc cross-references. The task's shape mirrors [T-016](T-016-mmu-activation.md) (which bundled HAL trait extension + BSP impl + bootstrap routine + audit entries + cross-references in one task); the implementation may land across multiple commits within the same task.

## Acceptance criteria

- [ ] **ADR-0035 Accepted** before code lands. Same-day Accept after careful re-read is permitted per [ADR-0025 §Revision notes](../../../decisions/0025-adr-governance-amendments.md); Propose commit is separate from the Accept commit per [`write-adr` skill §10](../../../../.agents/skills/write-adr/SKILL.md).
...
- [ ] **`Pmm::new(extent, reserved)`** constructor — `pub fn new(extent: PhysFrameRange, reserved: &[PhysFrameRange]) -> Result<Self, PmmError>`. Performs five **fail-fast** validations before any bitmap mutation: (i) `extent.start` and `extent.end` are `PAGE_SIZE`-aligned (returns `Err(PmmError::MisalignedAddress)` otherwise; the row-1 `PhysFrame::from_aligned` unwrap depends on this structurally rather than on BSP discipline); (ii) the bitmap covers the extent (`extent.frame_count() <= N * 8`; returns `Err(PmmError::OutOfRange)` otherwise — BSP picked too small an `N`, surfaced here rather than as a silent buffer overflow at the per-range bit-set step); (iii) `reserved.len() <= R` (returns `Err(PmmError::TooManyReservedRanges)` otherwise); (iv) each reserved range is page-aligned, fits inside `[extent.start, extent.end)`, and is non-inverted (`range.end >= range.start`) — returns `Err(PmmError::MisalignedAddress)` or `Err(PmmError::OutOfRange)` otherwise; without this the per-range bit-set loop could write past the bitmap's `[u8; N]` bound or compute non-integer frame indices; (v) no two reserved ranges overlap (pairwise half-open check; touching boundaries `[a, b)` + `[b, c)` are accepted) — returns `Err(PmmError::OverlappingReservedRanges)` otherwise; the round-1 PR #26 review added this gate to prevent bitmap-vs-counter drift when two ranges share frames (the shared bit gets set once but each entry double-counts `reserved_count`, leaving `stats()` inconsistent with the bitmap). After all five pass, walks the reserved-range list and sets every covered frame's bit to 1 (Reserved); copies the list into the `reserved_ranges` array (remaining slots `None`) for `free_frame`'s defensive validation; sets `hint` to the first frame *not* in any reserved range; computes `free_count` / `reserved_count` / `allocated_count` initial values. **Safe Rust**; no `unsafe`. Per [ADR-0035 §Simulation §Step 0](../../../decisions/0035-physical-memory-manager.md#simulation).
...
- [ ] **`PmmError` enum** — `#[non_exhaustive]`. Variants: `OutOfRange` (frame PA outside the managed `extent`, or `extent.frame_count()` exceeds the bitmap capacity `N * 8`, or a reserved range falls outside the extent or is inverted), `MisalignedAddress` (frame PA not 4 KiB-aligned — should be unreachable through `PhysFrame` construction but defensively named), `DoubleFree` (frame was already Free or its PA falls in a reserved range), `TooManyReservedRanges` (caller passed more than `R` ranges to `Pmm::new`), `OverlappingReservedRanges` (round-1 PR #26 addition; two reserved ranges share at least one frame, which would cause bitmap-vs-counter drift).
...
- [ ] **`new_rejects_too_many_reserved_ranges`** — `Pmm::new(extent, &[range × (R + 1)])` returns `Err(TooManyReservedRanges)`; the bitmap is not partially mutated.
...
- [ ] **`new_rejects_unaligned_extent`** — pin the [§Simulation §Step 0](../../../decisions/0035-physical-memory-manager.md#simulation) extent-alignment fail-fast: `Pmm::new(PhysFrameRange { start: PhysAddr(0x4000_0001), end: PhysAddr(0x4800_0000) }, &[])` (start not page-aligned) returns `Err(PmmError::MisalignedAddress)`; the bitmap is not partially mutated. Repeat for `extent.end` not page-aligned. Without this validation the row-1 `from_aligned` unwrap would silently misbehave or panic.
- [ ] **`new_rejects_reserved_range_outside_extent`** — pin the §Step 0 reserved-range-bounds fail-fast: `Pmm::new(extent, &[range_partially_outside])` returns `Err(PmmError::OutOfRange)`; `Pmm::new(extent, &[range_entirely_above_extent])` likewise. Bitmap is not partially mutated. Without this the per-range bit-set loop could write past the bitmap's `[u8; N]` bound.
- [ ] **`free_frame_rejects_pa_outside_extent`** — pin the §Step 2 extent-bounds fail-fast: `free_frame(PhysFrame whose PA < extent.start)` returns `Err(PmmError::OutOfRange)` without computing any index; `free_frame(PhysFrame whose PA >= extent.end)` likewise. Bitmap unchanged. Distinct from the reserved-range-rejection test (which covers PAs *inside* extent that fall in a reserved range).

### Audit-log update (`docs/audits/unsafe-log.md`)

- [ ] **UNSAFE-2026-0026 (new audit-log entry)** — adds the PMM frame-zeroing site as its own entry, NOT an Amendment of UNSAFE-2026-0001, per ADR-0035 §Dependency chain step 5's adjudication-deferred verdict (PL011 MMIO base blessing and PMM RAM zero-fill differ enough on what they touch / what proves ownership / what the failure mode is that the umbrella stretch would violate unsafe-policy.md §3's "same safety argument" requirement). The entry names: (a) location (`kernel/src/mm/pmm.rs::Pmm::alloc_frame`); (b) operation (`core::ptr::write_bytes` over a 4 KiB region whose PA is identity-mapped to a kernel-readable VA per ADR-0027); (c) invariants (page-alignment of target, exclusive ownership at write time via just-set bitmap bit, identity mapping post-MMU, bitmap-math overflow-freedom, `write_bytes` ordering); (d) rejected alternatives (slice-from-raw-parts-mut → same unsafe in different syntax; lazy zero on page-fault → no v1 page-fault routing; skip-zero → FrameProvider contract + B5+ isolation hazard; volatile_set_memory → no peer observer; hypothetical Mmu::zero_frame trait method → relocates audit point, doesn't remove).
- [ ] **All `unsafe` discipline** ([unsafe-policy.md §1](../../../standards/unsafe-policy.md)) holds: the `unsafe` block in `Pmm::alloc_frame` carries a `// SAFETY:` comment naming the audit reference (UNSAFE-2026-0026) with inline rejected-alternatives summary.
...
- [ ] `cargo host-test` passes — expected ~198 (current 185 + 13 new PMM tests: 8 core scenarios + 2 reserved-range guards + 3 fail-fast validation guards from PR #25 round-3 review per ADR-0035 §Simulation §Step 0 + §Step 2 — `new_rejects_unaligned_extent`, `new_rejects_reserved_range_outside_extent`, `free_frame_rejects_pa_outside_extent`).
...
1. **`kernel/src/mm/pmm.rs` — `Pmm` struct + bitmap arithmetic + `Pmm::new` constructor.** Reservation marking; counter init; hint-pointer init; reserved-range cache copy; five fail-fast validations (extent-alignment, bitmap-size, `R`-cap, per-range {alignment + bounds + non-inverted}, pairwise-overlap — the last gate added in PR #26 round-1 review). Pure safe Rust; bitmap operations are `[u8]` integer arithmetic. Host tests added: `new_marks_reserved_ranges_and_initialises_counters` + `new_rejects_too_many_reserved_ranges` + `new_rejects_unaligned_extent` + `new_rejects_reserved_range_outside_extent` + `new_rejects_overlapping_reserved_ranges` (all five pin `Pmm::new`, so they land in the constructor commit). No `unsafe` in this commit.
2. **`kernel/src/mm/pmm.rs` — `alloc_frame` / `free_frame` / `stats`.** Adds the frame-zeroing `unsafe` block covered by **UNSAFE-2026-0026 (new audit-log entry per the §Dependency-chain-step-5 adjudication-deferred verdict)**. Host tests added: alloc/free round-trip + `OutOfFrames` + `DoubleFree` rejection (free + reserved-range) + populated-slots-only defensive scan + `free_frame_rejects_pa_outside_extent` extent-bounds fail-fast + recovery-after-free + `stats` parity. The new entry lands in `docs/audits/unsafe-log.md` in this commit.
...
- **Why one bundled task instead of T-017a + T-017b + T-017c?** Mirrors [T-016](T-016-mmu-activation.md) which bundled HAL trait extension + BSP impl + bootstrap routine + audit entries + cross-references in one task. The split would be artificial: PMM module + BSP wiring + audit-log entry (UNSAFE-2026-0026, per the §Dependency-chain-step-5 adjudication) + doc updates are tightly coupled at the implementation level. A single task with four bisectable commits is cleaner than three tasks each with one or two commits.
...
| 2026-05-09 | @cemililik (+ Claude Opus 4.7 agent) | ADR-0035 `Accepted` (PR #25 merged). Status `Draft → In Progress`; implementation begins on branch `t-017-physical-memory-manager`. |
| 2026-05-10 | @cemililik (+ Claude Opus 4.7 agent) | Status `In Progress → Done`. All four bisectable commits land green (`cargo fmt --check`, `cargo host-clippy -D warnings`, `cargo kernel-clippy -D warnings`, `cargo test`, `cargo +nightly miri test`, `cargo kernel-build`); 199/199 host tests + 14/14 PMM-specific miri (Stacked Borrows clean on the frame-zeroing raw-pointer path); QEMU smoke trace gains exactly one new line `tyrne: pmm initialized (32604 frames available; 164 reserved)` immediately after `tyrne: mmu activated` and before `tyrne: timer ready (...)`. Total = 32 768 frames; reserved = 164 (128 firmware + 36 kernel image / `.bss` / `.boot_pt` / stack); free = 32 604. `-d int,unimp,guest_errors` shows only the pre-existing PL011 "data written to disabled UART" warning (379 → 443; +64 ≈ bytes of the new banner line). **Audit-log adjudication**: UNSAFE-2026-0026 lands as a **new entry** (not a UNSAFE-2026-0001 Amendment) per ADR-0035 §Dependency chain step 5's adjudication-deferred caveat — PL011 MMIO base blessing and PMM RAM zero-fill share surface shape ("kernel-static / identity-mapped raw-pointer") but differ on what they touch, what proves ownership, and what the failure mode is, so an Amendment would stretch the umbrella beyond unsafe-policy.md §3's "same safety argument" requirement. The full Rejected-alternatives walk in UNSAFE-2026-0026 covers slice-from-raw-parts (same unsafe, different syntax), lazy zero on page-fault (no v1 page-fault routing), skip-zero (FrameProvider contract + B5+ isolation hazard), volatile_set_memory (no peer observer), and a hypothetical new `Mmu::zero_frame` trait method (relocates audit point, doesn't remove). ADR-0035 §Positive consequence "Zero new `unsafe` audits" flips to "one new entry" per the adjudication caveat; ADR-0035 itself stays byte-stable. |
