...
- [ ] **New file `bsp-qemu-virt/src/mmu_bootstrap.rs`** containing `unsafe fn mmu_bootstrap()` — the once-per-boot Rust function called by `kernel_entry` between the timer banner and the GIC initialisation. Implements the [ADR-0027 §Simulation](../../../decisions/0027-kernel-virtual-memory-layout.md#simulation) sequence:
...
- [ ] **`kernel_entry` wired**: call `mmu_bootstrap()` between [`bsp-qemu-virt/src/main.rs:621`](../../../../bsp-qemu-virt/src/main.rs#L621) (the `boot_ns = cpu.now_ns()` snapshot) and the GIC initialisation. The ordering matters: `mmu_bootstrap` must run *before* the first MMIO-attribute-sensitive access (GIC distributor / CPU interface writes), and *after* `cpu.now_ns()` is sampled (so the boot-to-end timing baseline includes MMU activation). Add a `tyrne: mmu activated` (or equivalent) marker line printed by `kernel_entry` immediately after the bootstrap returns, so the QEMU smoke trace gains a single new line documenting the activation.
...
- [ ] **QEMU smoke unchanged plus one new line.** The post-T-016 trace adds `tyrne: mmu activated` (or equivalent marker) between the timer banner and the IPC demo's first task line. Every other line of the trace is byte-for-byte identical to the post-T-015 baseline. The boot-to-end timing increases by the cost of `mmu_bootstrap` (estimated < 100 µs); the [P10 wall-clock harness](../../../analysis/reviews/code-reviews/2026-05-07-pr-12-to-17-multi-axis-review/track-d-perf.md) (landing in PR #20) is the canonical band.
...
- **Why `mmu_bootstrap` between the timer banner and the GIC initialisation?** GIC distributor / CPU interface writes are the first MMIO-attribute-sensitive operations (without device-nGnRnE attributes, speculative reads can produce phantom interrupts). The MMU must be on with the device attributes encoded before any MMIO write happens. The timer banner uses the UART, which is also MMIO — meaning the timer banner ought to come *after* `mmu_bootstrap` too. **Implementation note:** the order is `cpu.now_ns()` snapshot → `mmu_bootstrap()` → `tyrne: mmu activated` print → GIC init → timer banner. The boot-to-end timing baseline includes MMU activation cost; the timer banner moves to after the MMU is on so its UART writes go through device attributes.
