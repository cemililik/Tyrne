...
5. **Bootstrap AddressSpace** (constraint 4) is handled by a one-shot `AddressSpace::wrap_bootstrap(root: PhysFrame) -> AddressSpace<M>` constructor that **does not** call `Mmu::create_address_space` (the topology is already active per T-016). It simply names the already-live root frame. The BSP calls it exactly once at boot, after `mmu_bootstrap()` and after `Pmm::new` (so the bootstrap root frame is already marked Reserved in the PMM, and `wrap_bootstrap` does not consume a PMM allocation). All subsequent `AddressSpace` values are minted via `cap_create_address_space` → `PMM.alloc_frame()` → `Mmu::create_address_space(root)`.
...
- **No per-AS ASID isolation in v1.** Two AS coexisting + activation-without-TLB-flush relies on T-018's test fixture pre-installing all mappings before activation. A misbehaving caller that maps post-activation could observe stale TLB entries from the previous AS. *Mitigation:* the v1 demo has no userspace, so no untrusted caller can construct this scenario; T-018's host tests pin the "activate-after-map" ordering as a discipline rule; ADR-0033 will introduce ASID-based isolation when B5+ surfaces per-task `TTBR0_EL1` swap. The §Simulation row 3 cell explicitly names this carry-forward.
