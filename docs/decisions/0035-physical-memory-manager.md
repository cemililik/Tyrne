...
The PMM's design surface — allocation discipline, metadata layout, reservation tracking — is independent enough from the address-space data structure that it warrants a separate ADR (per the methodical-pace principle of CLAUDE.md non-negotiable #6). [ADR-0028](#) is reserved for the address-space data structure (per [ADR-0027 §Context](0027-kernel-virtual-memory-layout.md#context)); ADR-0035 settles the layer below it.
...
- **Audit-discipline minimisation.** The PMM today should add zero new audit-log entries. The frame-zeroing step is the only operation that potentially touches kernel-owned memory the caller doesn't yet own — but if the PMM can zero via safe `core::slice::fill` (over a `&mut [u8]` materialised through identity mapping), no `unsafe` is involved.
...
| 2 | Bit `i` = 1 (Allocated); `hint = i + 1`; `free_count` reduced | `free_frame(PhysFrame(...))` computes `i` from the frame's PA (`(pa - extent.start) / 4096`); validates the frame was Allocated (not Reserved) by reading the bit — an attempted free of a Reserved or never-Allocated frame returns `Err(PmmError::DoubleFree)` and leaves the bitmap unchanged; clears bit `i`; if `i < hint`, sets `hint = i` (rewind); increments `free_count` | Bit `i` = 0 (Free); `hint = min(prev_hint, i)`; `free_count` incremented | Frame is reclaimable; the rewind discipline keeps the hint pointing at the lowest known free frame, preventing fragmentation from accumulating linearly with alloc / free interleaving. **Critical row:** the Reserved-vs-Allocated discrimination is what prevents the bitmap from silently re-handing-out a kernel-image frame; the bitmap's single bit cannot distinguish Reserved from Allocated, so the PMM either tracks two bitmaps or accepts that "double free of a Reserved frame is undefined" (we choose the latter — `free_frame` against a Reserved frame is a kernel-programming error, which `PmmError::DoubleFree` reports). |
