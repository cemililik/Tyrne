...
- **2026-04-22 — T-007 implementation rider.** Option A's *Decision outcome* paragraph described idle's body as `cpu.wait_for_interrupt()` + `yield_now`. During T-007 implementation, QEMU smoke confirmed that this hangs the v1 demo: FIFO scheduling dispatches idle between two ready application tasks (e.g. after Task A's `ipc_send_and_yield` unblocks Task B and yields, the ready queue head becomes idle before B), and with **no IRQ source configured in v1**, `wfi` suspends the core indefinitely. The hang is a trace truncation at "Task A — sending IPC" — the demo never reaches B's reply.
  The fix lands in T-007: idle's body is `core::hint::spin_loop()` + `yield_now` for now. Shape and registration path are unchanged; only the suspend primitive is deferred. Option A's "yield_now's single-ready fast path collapses solo-idle to a tight WFI loop" claim is therefore **accurate only once a wake source exists**; until then idle spin-yields.
  This rider does not change the chosen option. It corrects the *Decision outcome*'s implied workload claim (WFI is cheap only when there is something to wake you up) and it records the T-006-retro's "trace the call graph" lesson applied in hindsight: the ADR assumed idle would run rarely and briefly, but FIFO places idle at the head of the ready queue on a regular cadence, making WFI's IRQ-source dependency load-bearing even for the two-task demo. **Cost of the adjustment:** one extra context switch per inter-task yield when idle happens to sit at the head — acceptable for v1.

  **Sub-rider — WFI activation requires *two* tasks, not one.** An earlier draft of this rider said "When T-009 wires a timer IRQ, idle's body becomes `wait_for_interrupt` + `yield_now`." That conflated two distinct concerns: the **time source** (read CNTVCT_EL0 to know what time it is) versus the **IRQ delivery** (route a generic-timer interrupt through the GIC + IVT to a handler). T-009's scope (phase-b.md §B0 item 5) is the time source only — *measurement-half* — landed 2026-04-23. WFI activation is gated on **[T-012](../analysis/tasks/phase-b/T-012-exception-and-irq-infrastructure.md)** (Draft, opened 2026-04-27 to ground this previously-handwaved forward-reference per the new ADR-0025 §Rule 1 (forward-reference contract)). T-012 will (a) program `CNTV_CTL_EL0` / `CNTV_CVAL_EL0`, (b) configure the GIC v2 distributor + CPU interface for the generic-timer IRQ line, (c) install an exception-vector table at `VBAR_EL1` and a handler-dispatch loop. When T-012 closes, `idle_entry`'s body switches from `spin_loop + yield_now` to `wait_for_interrupt + yield_now` and this rider's *Sub-rider* condition is fully met. Until T-012 lands, idle stays in the spin-yield form regardless of T-009's status. The ADR's chosen option still holds; the precondition for the option's full benefit is now correctly attributed to two sequential pieces of infrastructure rather than one.
...
- **2026-04-27 — pointer to architecture doc.** [T-008](../analysis/tasks/phase-b/T-008-architecture-docs.md) created [`docs/architecture/scheduler.md`](../architecture/scheduler.md), which synthesises this ADR's idle-task + `SchedError::Deadlock` design alongside [ADR-0019](0019-scheduler-shape.md) (FIFO ready queue), [ADR-0020](0020-cpu-trait-v2-context-switch.md) (`ContextSwitch` split), and [ADR-0021](0021-raw-pointer-scheduler-ipc-bridge.md) (raw-pointer bridge) into a single readable picture. The ADR body is unchanged; this rider provides the bidirectional cross-reference T-008's DoD asks for.

- **2026-04-28 — first-rider Sub-rider closes (T-012, commit `28c5ce9`).** The 2026-04-22 first rider's *Sub-rider* set the WFI-activation precondition as the conjunction "T-009 (time source) **and** T-012 (IRQ delivery)". T-009 closed on 2026-04-23 (commit `39fb66c`); [T-012](../analysis/tasks/phase-b/T-012-exception-and-irq-infrastructure.md) closes today, fulfilling both halves:
  - **Time source** — `<QemuVirtCpu as Timer>::now_ns` reads `CNTVCT_EL0` (T-009 / [UNSAFE-2026-0015](../audits/unsafe-log.md#unsafe-2026-0015--generic-timer-system-register-reads-cntpct_el0-cntfrq_el0)).
  - **IRQ delivery** — the GIC v2 distributor + CPU interface is initialised and routes PPI 27 to the EL1 vector table at `tyrne_vectors+0x280`, where [`irq_entry`](../../bsp-qemu-virt/src/exceptions.rs) acknowledges, masks the timer at the source, and signals EOI ([UNSAFE-2026-0019](../audits/unsafe-log.md#unsafe-2026-0019--qemuvirtgic-mmio-surface), [UNSAFE-2026-0020](../audits/unsafe-log.md#unsafe-2026-0020--el1-exception-vector-table-install--asm-trampolines), [UNSAFE-2026-0021](../audits/unsafe-log.md#unsafe-2026-0021--el1-virtual-generic-timer-compare-register-writes)).
  - **`Timer::arm_deadline` + `Timer::cancel_deadline`** are now non-stub on `QemuVirtCpu` (see [ADR-0010 Revision notes](0010-timer-trait.md#revision-notes)), so a wake source exists in principle even though v1's cooperative IPC demo does not arm any deadline.

  Per the Sub-rider's closure condition, [`idle_entry`](../../bsp-qemu-virt/src/main.rs)'s body switches from `core::hint::spin_loop()` + `yield_now` to `cpu.wait_for_interrupt()` + `yield_now`. This is the form Option A's *Decision outcome* originally specified — the spin-yield interim form is now retired. The same FIFO scheduling dynamics the first rider described still hold: idle sits at the head of the ready queue between application tasks, but the WFI is now *cheap* (the core enters low-power wait until the next architected wake event) rather than a pure power-burning busy-loop. v1's cooperative IPC demo has no timer arms today, so in practice idle's `wait_for_interrupt` will not be woken until T-012's IRQ-armed half is exercised by a future task arc — but the kernel-level discipline matches the ADR's original chosen shape from this commit forward.

  The ADR body is unchanged; this rider records that the load-bearing precondition is met and the implementation now matches the spec. The first rider and Sub-rider remain in place above as the historical record of why the implementation went through a spin-yield interim form.
