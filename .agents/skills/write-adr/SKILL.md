...
5. **Fill the body:**
   ...
   - **Decision outcome → Simulation** (for multi-step state-machine ADRs). A subsection containing a 3–5 row table walking the worst-case interaction through the chosen shape — `(state-pre, action, state-post, switch target / observable effect)`. Required for ADRs whose subject is a state machine: capability flows (cap_copy / cap_derive / cap_revoke), IPC handshakes (send / recv / notify), scheduler dispatch (yield_now / start / IPC bridge), exception / IRQ entry, MMU / TLB transitions, syscall ABI handshakes. Omitted with a one-line "Not applicable — this ADR settles a single-shape decision" note for process / governance / dependency-policy / single-decision ADRs. Codified after [ADR-0026](../../../docs/decisions/0026-idle-dispatch-fallback.md)'s table caught what [ADR-0022](../../../docs/decisions/0022-idle-task-and-typed-scheduler-deadlock.md)'s prose-only reasoning had missed; the [2026-05-07 B1 closure retro §"What we learned"](../../../docs/analysis/reviews/business-reviews/2026-05-07-B1-closure.md) is the source.
   - **Decision outcome → Simulation row-to-verification mapping** (for multi-step state-machine ADRs). Each simulation row must map to a *specific* verification artefact in the implementing task: a host test pinning the transition behaviour for pure-Rust transitions, or an audit-log entry (UNSAFE-YYYY-NNNN) covering the `unsafe`-touching step (or both, for transitions whose verification spans both layers). The implementing task's review-history row records this mapping explicitly — for example `row 0 → wrap_bootstrap_returns_address_space_with_root host test; row 3 → yield_now_activates_when_tasks_differ_in_address_space + UNSAFE-2026-0014 5th Amendment`. **Rationale:** prose-only ADR review cannot catch off-by-one transition errors; pairing every row with executable / audited verification forces the implementer to discharge each simulated state-change with a concrete artefact and surfaces gaps before merge. The pattern is stable at four data points — ADR-0026 row → T-014's `unblock_after_yield_dispatches_unblocked_receiver_not_idle` test; ADR-0032 rows → T-015's `ipc_cancel_recv` regression tests + UNSAFE-2026-0014 4th Amendment; [ADR-0027](../../../docs/decisions/0027-kernel-virtual-memory-layout.md) rows → T-016's `vmsav8` encoder host tests + UNSAFE-2026-0022 / 0023 / 0024 / 0025 audit entries; [ADR-0028](../../../docs/decisions/0028-address-space-data-structure.md) rows → T-018's `address_space::tests` + UNSAFE-2026-0014 5th Amendment. The codification was deferred for two closure trios (B2 → B3) before the [B3 closure retro §Adjustments](../../../docs/analysis/reviews/business-reviews/2026-05-14-B3-closure.md#adjustments) re-triggered it.
...
- [ ] **For multi-step state-machine ADRs:** Decision outcome includes a *Simulation* subsection with a 3–5 row table walking the worst-case interaction through the chosen shape. Other ADR subjects: a one-line "Not applicable" suffices.
- [ ] **For multi-step state-machine ADRs:** every simulation row maps to a verification artefact (host test or UNSAFE-YYYY-NNNN audit entry) in the implementing task; the mapping is recorded in the task's review-history row. Skill anti-pattern: a Simulation table whose rows have no corresponding test / audit entry — the table becomes documentation drift instead of executable verification.
...
- **§Simulation table without row-to-verification mapping.** A simulation row with no corresponding host test or audit-log entry is documentation drift, not executable verification. The mapping discipline (Procedure step 5 sub-bullet) must be discharged by the implementing task and recorded in its review-history row.
